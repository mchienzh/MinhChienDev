...
-- ============================================
if not game then 
    error("Script không chạy ngoài Roblox") 
end
...
if not LocalPlayer then 
    error("LocalPlayer nil – inject thất bại") 
end

-- ============================================
-- LOAD RAYFIELD (để dùng Notify)
-- ============================================
...
-- ============================================
-- HÀM KIỂM TRA API KEY (gọi Gemini test)
-- ============================================
local function ValidateKey(key)
    ...
    elseif syn_request then requestFunc = syn_request
        ...
        Rayfield:Notify({Title = "LỖI", Content = "Không tìm thấy request – executor lỗi", Duration = 3})
        return false 
    end
    ...
    if not ok then 
        Rayfield:Notify({Title = "LỖI MẠNG", Content = "Không thể kết nối đến Gemini", Duration = 3})
        return false 
    end
    
    if res.StatusCode == 200 then
        Rayfield:Notify({Title = "✅ KEY HỢP LỆ", Content = "API key hoạt động tốt!", Duration = 2})
        return true
    elseif res.StatusCode == 401 or res.StatusCode == 403 then
        Rayfield:Notify({Title = "❌ KEY KHÔNG HỢP LỆ", Content = "Sai key hoặc hết hạn", Duration = 3})
        return false
    else
        Rayfield:Notify({Title = "⚠️ LỖI KHÔNG XÁC ĐỊNH", Content = "HTTP " .. res.StatusCode, Duration = 3})
        return false
    end
end

-- ============================================
-- TẠO POPUP NHẬP KEY (đặt giữa màn hình)
-- ============================================
...
DescLabel.Text = "Nhập API key từ Google AI Studio"
...
KeyBox.Position = UDim2.new(0, 20, 0, 72)
...
local function OnCheck()
    ...
    task.spawn(function()
        ...
        if valid then
            API_KEY = key
            KEY_VALIDATED = true
            StatusLabel.Text = "✅ Key hợp lệ! Đang khởi động bot..."
            StatusLabel.TextColor3 = Color3.fromRGB(50, 205, 50)
            -- Ẩn popup sau 1s
            task.wait(1)
            ScreenGui:Destroy()
            -- Gọi hàm khởi tạo bot chính
            InitializeBot()
        else
            StatusLabel.Text = "❌ Key không hợp lệ, thử lại!"
            StatusLabel.TextColor3 = Color3.fromRGB(255, 80, 80)
...
CancelButton.MouseButton1Click:Connect(function()
    ...
    -- Có thể dừng script hoặc cho phép nhập sau, nhưng thôi
end)

-- ============================================
-- HÀM KHỞI TẠO BOT (CHẠY SAU KHI KEY VALID)
-- ============================================
function InitializeBot()
    -- CẤU HÌNH TỔNG (dùng API_KEY đã nhập)
    local CONFIG = {
        API_KEY = API_KEY,
        PREFIX = "!ai ",
    ...
    local function BuildPrompt(question, pName)
        ...
        return context .. "Trợ lý AI thân thiện. Trả lời Tiếng Việt, ngắn. Câu hỏi: " .. question
    end
...
-- ============================================
-- LƯU Ý: SCRIPT SẼ DỪNG Ở ĐÂY CHO ĐẾN KHI KEY HỢP LỆ
-- ============================================
-- Popup đã hiện, chờ người dùng nhập và xác thực.
-- Khi hợp lệ, InitializeBot() sẽ được gọi.