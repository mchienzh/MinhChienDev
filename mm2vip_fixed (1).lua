...
local function ShootMurdererRemote()
    ...
    -- ── 1. Fire every RemoteEvent inside the tool ────────────────────────────
    -- Try all known MM2 gun remote signatures in order of likelihood:
    --   (a) mouseHit CFrame alone          – most common in current MM2 builds
    --   (b) mouseHit + Head part           – gun scripts that read mouse.Target
    --   (c) mouseHit + HRP                 – alternative part target
    --   (d) hitPos Vector3                 – stripped-down server scripts
    --   (e) origin Vector3 + hitPos Vector3 – some custom gun remotes
    --   (f) Ray                            – legacy / older builds
    for _, remote in pairs(tool:GetDescendants()) do
        if remote:IsA("RemoteEvent") then
            pcall(function() remote:FireServer(mouseHit) end)                     -- (a)
            pcall(function() remote:FireServer(mouseHit, tHead or tRoot) end)    -- (b)
            pcall(function() remote:FireServer(mouseHit, tRoot) end)             -- (c)
            pcall(function() remote:FireServer(hitPos) end)                      -- (d)
            pcall(function() remote:FireServer(origin, hitPos) end)              -- (e)
            pcall(function() remote:FireServer(Ray.new(origin, dir * 1000)) end) -- (f)
        end
    end

    -- ── 2. Named remotes in ReplicatedStorage ───────────────────────────────
    for _, remote in pairs(ReplicatedStorage:GetDescendants()) do
        if remote:IsA("RemoteEvent") then
            local n = remote.Name:lower()
            if n:find("shoot") or n:find("fire") or n:find("gun")
               or n:find("bullet") or n:find("sheriff") then
                pcall(function() remote:FireServer(mouseHit) end)
                pcall(function() remote:FireServer(mouseHit, tHead or tRoot) end)
                pcall(function() remote:FireServer(origin, hitPos) end)
            end
        ...
        pcall(function()
            for _, v in pairs(getnilinstances()) do
                if v.ClassName == "RemoteEvent" then
                    ...
                    if n:find("shoot") or n:find("fire") or n:find("gun")
                       or n:find("bullet") or n:find("sheriff") then
                        pcall(function() v:FireServer(mouseHit) end)
                        pcall(function() v:FireServer(mouseHit, tHead or tRoot) end)
                    end
                end
            end
        end)
    ...
    -- ── 4. Activate tool (simulates mouse click) ─────────────────────────────
    -- Snap camera toward hitPos so the gun LocalScript reads the correct
    -- mouse.Hit at the moment of Activate().  Restore immediately after
    -- to avoid locking the player's camera.
    local oldCamType = Camera.CameraType
    local oldCF      = Camera.CFrame
    pcall(function()
        Camera.CameraType = Enum.CameraType.Scriptable
