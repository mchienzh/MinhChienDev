...
local function FlingMurderer()
    ...
    -- Tìm sát thủ gần nhất còn sống
    local murderer, bestDist = nil, math.huge
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character then
            local r = plr.Character:FindFirstChild("HumanoidRootPart")
            local h = plr.Character:FindFirstChild("Humanoid")
            if r and h and h.Health > 0 and getPlayerRole(plr) == "murderer" then
                local d = (myRoot.Position - r.Position).Magnitude
    ...
    if not murderer or not murderer.Character then
        print("[Fling] Không tìm thấy sát thủ")
        return
    end
    ...
    flingActive = true
    local savedCFrame = myRoot.CFrame       -- lưu vị trí cũ để teleport về
    local origMurdererPos = mRoot.Position

    print("[Fling] ➡ Teleport đến sát thủ: " .. murderer.Name)

    -- Bước 1: Teleport đè lên sát thủ
    myRoot.CFrame = mRoot.CFrame

    -- Bước 2: Tắt collision toàn bộ character (để xuyên qua địa hình khi quay)
    ...
    -- Bước 3: Gắn BodyAngularVelocity + BodyVelocity max tốc độ
    ...
    -- Bước 4: Vòng spin – quay nhân vật liên tục quanh sát thủ trên mọi trục
    local spinConn
    local t = 0
    spinConn = RunService.Heartbeat:Connect(function(dt)
        t = t + dt * 25   -- tốc độ quay cực cao

        local mChar = murderer.Character
        if not mChar then return end
        local mr = mChar:FindFirstChild("HumanoidRootPart")
        if not mr then return end

        -- Xoay CFrame nhân vật theo 3 trục xung quanh vị trí sát thủ
        local spinCF = CFrame.new(mr.Position) * CFrame.Angles(
            math.sin(t)           * math.pi * 2,
            math.cos(t * 1.37)    * math.pi * 2,
            math.sin(t * 0.71 + 1)* math.pi * 2
        )
        pcall(function()
            myRoot.CFrame = spinCF

            -- AngularVelocity đổi chiều liên tục → lực ly tâm mọi hướng
            bav.AngularVelocity = Vector3.new(
                math.sin(t)         * 9999,
                math.cos(t * 1.37)  * 9999,
                math.sin(t * 0.71)  * 9999
            )

            -- BodyVelocity cũng đổi chiều để tạo thêm lực đẩy
    ...
    -- Bước 5: Đợi đến khi sát thủ bị bắn ra đủ xa HOẶC timeout 10 giây
    local timeout = 10
    local elapsed = 0
    local flung   = false
    ...
    while elapsed < timeout do
        ...
        elapsed = elapsed + 0.1
        local mChar = murderer.Character
        if not mChar then flung = true; break end
        local mr = mChar:FindFirstChild("HumanoidRootPart")
        if not mr then flung = true; break end
        local mh = mChar:FindFirstChild("Humanoid")
        if mh and mh.Health <= 0 then flung = true; break end
        -- Kiểm tra đã bay xa >400 studs hoặc rơi xuống void (Y < -100)
        local dist = (mr.Position - origMurdererPos).Magnitude
        if dist > 400 or mr.Position.Y < -100 then
            flung = true; break
        end
    end

    -- Bước 6: Dọn dẹp physics objects
    spinConn:Disconnect()
    pcall(function() bav:Destroy() end)
    pcall(function() bv:Destroy() end)

    -- Reset velocity trước khi teleport về
    pcall(function()
    ...
    if flung then
        print("[Fling] ✅ Sát thủ đã bị bắn ra xa!")
    else
        print("[Fling] ⏱ Timeout – sát thủ chưa bị bắn đủ xa")
    end
end
