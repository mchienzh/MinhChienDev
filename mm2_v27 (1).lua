-- ================================================================
-- MURDER MYSTERY 2 – ULTIMATE V27 (Delta X)
-- FIX v27:
--   [1] Float Buttons: Drag có ngưỡng (DRAG_THRESHOLD=10px) –
--       kéo = drag, nhẹ tay = tap → tách biệt hoàn toàn
--   [2] Shoot Killer: teleport sát bên cạnh → bắn remotes →
--       firetouchinterest → Activate → quay về (không đổi CameraType)
--   [3] Kill All: thu thập tất cả target, xếp hình quạt phía trước
--       CÙNG LÚC, rồi task.spawn từng kill song song (không delay)
--   [4] Lock Aim: snap mỗi frame (không lerp), bỏ giới hạn role
--       + Gun + Tap màn hình → Freeze sát thủ 0.6s (anchor + lock)
-- ================================================================
...
-- ================================================================
-- ===== LOCK AIM – SNAP TỐC ĐỘ CAO + FREEZE ON TAP =====
-- ================================================================

-- Helper: tìm sát thủ gần nhất khi LockAim bật
local function findLockAimMurderer()
    local myChar = LocalPlayer.Character
    local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
    if not myRoot then return nil end
    local best, bestDist = nil, math.huge
...
-- RenderStepped: snap camera về hướng sát thủ mỗi frame (bất kể role)
-- Tốc độ snap = 1 frame (~1/60s) → không có murderer nào đủ nhanh thoát được
RunService.RenderStepped:Connect(function()
    if not features.LockAim then return end

    local murderer = findLockAimMurderer()
    ...
    -- Snap tức thì, không lerp, không đổi CameraType
    Camera.CFrame = CFrame.new(Camera.CFrame.Position, aimPos)
end)

-- ── FREEZE ON TAP ────────────────────────────────────────────────
-- Gun equipped + chạm màn hình → đóng băng sát thủ 0.6 giây
local freezeActive = false

UserInputService.InputBegan:Connect(function(input, processed)
    if processed then return end
    -- Chỉ bắt Touch (mobile) hoặc MouseButton1 (PC) khi LockAim bật
    if input.UserInputType ~= Enum.UserInputType.Touch
       and input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
    if not features.LockAim then return end
    if freezeActive then return end

    -- Kiểm tra đang cầm súng
    local myChar = LocalPlayer.Character
    if not myChar then return end
    local heldTool = myChar:FindFirstChildOfClass("Tool")
    if not heldTool or not isGunName(heldTool.Name) then return end
    ...
    freezeActive = true
    local frozenCF  = mRoot.CFrame
    local oldSpeed  = mHum.WalkSpeed
    local wasAnchor = mRoot.Anchored

    -- Freeze: anchor + zero velocity + lock WalkSpeed
    pcall(function()
    ...
    print("[LockAim] ❄ Freeze sát thủ 0.6s")

    -- Trong 0.6s: giữ vị trí đóng băng mỗi frame
    local endTime = tick() + 0.6
    task.spawn(function()
        while tick() < endTime do
            if murderer.Character and mRoot and mRoot.Parent then
                pcall(function()
                    mRoot.CFrame    = frozenCF
                    mRoot.Velocity  = Vector3.new(0, 0, 0)
                end)
            else break end
            task.wait()
        ...
        -- Unfreeze
        pcall(function()
            mRoot.Anchored = wasAnchor
            mHum.WalkSpeed = oldSpeed
        end)
        freezeActive = false
        print("[LockAim] ✅ Unfreeze sát thủ")
    end)
end)
-- ─────────────────────────────────────────────────────────────────
...
-- ================================================================
-- ===== SHOOT SÁT THỦ – INSTANT TELEPORT-SHOOT-RETURN =====
-- ================================================================
local function ShootMurdererRemote()
        ...
        if bp then
            for _, t in pairs(bp:GetChildren()) do
                if t:IsA("Tool") and isGunName(t.Name) then
                    ...
                    task.wait(0.15) -- nhanh hơn v26 (0.25 → 0.15)
    ...
    if not tool or not isGunName(tool.Name) then
        print("[Shoot] Không có súng"); return
    end

    -- Tìm sát thủ gần nhất → fallback player gần nhất
    local target, bestDist = nil, math.huge
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character then
            local r   = plr.Character:FindFirstChild("HumanoidRootPart")
            local hum = plr.Character:FindFirstChild("Humanoid")
            if r and hum and hum.Health > 0 and getPlayerRole(plr) == "murderer" then
                local d = (myRoot.Position - r.Position).Magnitude
    ...
    -- ── INSTANT: Teleport sát bên cạnh sát thủ → bắn → quay về ──────
    local oldCF     = myRoot.CFrame
    local snapCF    = tRoot.CFrame * CFrame.new(0, 0, 1.8) -- đứng ngay sau lưng
    pcall(function() myRoot.CFrame = snapCF end)
    task.wait(0.02)  -- 1 frame stabilise

    local origin    = myRoot.Position + Vector3.new(0, 1.5, 0)
    local targetPos = tRoot.Position  + Vector3.new(0, 1.5, 0)
    local dir       = (targetPos - origin).Unit

    -- 1) Fire RemoteEvent trong Tool
    for _, remote in pairs(tool:GetDescendants()) do
        if remote:IsA("RemoteEvent") then
            pcall(function() remote:FireServer(origin, targetPos) end)
        ...
        pcall(function()
            for _, v in pairs(getnilinstances()) do
                if v.ClassName == "RemoteEvent" then
                    ...
                    if n:find("shoot") or n:find("fire") or n:find("gun") or n:find("bullet") then
                        pcall(function() v:FireServer(CFrame.new(origin, targetPos), CFrame.new(targetPos)) end)
                    end
                end
            end
        end)
    ...
    -- 4) firetouchinterest: súng chạm nạn nhân
    local gunHandle = tool:FindFirstChild("Handle") or tool:FindFirstChildOfClass("BasePart")
    if gunHandle then
        for _, vp in pairs(target.Character:GetDescendants()) do
            if vp:IsA("BasePart") then
                pcall(function() firetouchinterest(gunHandle, vp, 0) end)
                pcall(function() firetouchinterest(gunHandle, vp, 1) end)
            end
        end
    end

    -- 5) Activate tool (không đổi CameraType → không khoá màn hình)
    pcall(function() tool:Activate() end)

    -- Quay về vị trí cũ ngay lập tức
    task.delay(0.05, function() pcall(function() myRoot.CFrame = oldCF end) end)

    print(string.format("[Shoot] ✅ Bắn %s (%.1f studs)", target.Name, bestDist))
end

-- ================================================================
-- ===== KILL ALL – ĐỒNG THỜI (SIMULTANEOUS) =====
-- ================================================================
local killAllRunning = false

local function KillAll()
    if killAllRunning then print("[KillAll] Đang chạy, bỏ qua..."); return end
    killAllRunning = true
    ...
    -- Bước 2: Thu thập tất cả mục tiêu còn sống
    local targets = {}
    for _, plr in pairs(Players:GetPlayers()) do
    ...
    print("[KillAll] ⚡ Đồng thời mang " .. #targets .. " người đến trước mặt...")

    -- Bước 3: CÙNG LÚC mang tất cả player đến trước mặt (xếp hình quạt)
    local count = #targets
    for i, plr in pairs(targets) do
        ...
        -- Xếp hình quạt 180° phía trước để không chồng nhau
        local angle = (count == 1) and 0
            or (((i - 1) / (count - 1)) - 0.5) * math.pi * 1.2
        local radius = math.max(2, count * 0.6)
        local offsetX =  math.sin(angle) * radius
        local offsetZ = -math.cos(angle) * radius
        local frontCF = myRoot.CFrame * CFrame.new(offsetX, 0, offsetZ)
        pcall(function() tRoot.CFrame = frontCF end)
    end

    task.wait(0.06) -- 1 frame ổn định vị trí

    -- Bước 4: ĐỒNG THỜI chém tất cả bằng task.spawn
    for _, plr in pairs(targets) do
        task.spawn(function()
            ...
            if getnilinstances then
                pcall(function()
                    for _, v in pairs(getnilinstances()) do
                        if v.ClassName == "RemoteEvent" then
                            local n = v.Name:lower()
                            if n:find("knife") or n:find("kill") or n:find("stab")
                               or n:find("slash") or n:find("attack") or n:find("melee")
            ...
            -- firetouchinterest: dao chạm vào mọi part nạn nhân
            if knifeHandle then
                for _, vp in pairs(tChar:GetDescendants()) do
                    if vp:IsA("BasePart") then
            ...
            print(string.format("[KillAll] ⚔ %s", plr.Name))
        end)
    end
...
-- ================================================================
-- ===== AUTO PICK COIN – ANTI-KICK =====
-- Nguyên nhân bị kick: teleport liên tục đến nhiều đồng xu trong
-- cùng 1 vòng lặp, không có cooldown giữa các lần dịch chuyển.
-- Fix:
--   • Mỗi chu kỳ chỉ nhặt 1 đồng xu gần nhất (không duyệt hết)
--   • Cooldown 1.5s giữa hai lần nhặt liên tiếp
--   • Di chuyển mượt (Lerp 8 bước) thay vì teleport tức thì
--   • Thử fire ProximityPrompt/ClickDetector TRƯỚC khi di chuyển
--   • Quét mỗi 1.2s (không dồn dập 0.5s)
--   • Khoảng cách tối đa 18 studs (giảm từ 30)
-- ================================================================
local COIN_SCAN_INTERVAL = 1.2   -- giây giữa các lần quét
local COIN_PICK_COOLDOWN = 1.5   -- giây bắt buộc giữa hai lần nhặt
local COIN_MAX_DIST      = 200    -- studs: bán kính tìm coin
local COIN_STEPS         = 8     -- số bước Lerp di chuyển mượt
local COIN_STEP_WAIT     = 0.06  -- giây giữa mỗi bước
...
task.spawn(function()
    while true do
        ...
        -- Cooldown check: chưa đủ thời gian thì bỏ qua lần này
        if tick() - coinLastPick < COIN_PICK_COOLDOWN then continue end

        -- Tìm 1 đồng xu GẦN NHẤT (không nhặt hết trong 1 vòng)
        ...
        -- Đánh dấu đã nhặt (cooldown bắt đầu tính)
        coinLastPick = tick()

        local targetPos = nearest.Position + Vector3.new(0, 2.5, 0)

        -- Bước 1: Thử fire KHÔNG di chuyển trước (nếu không bị chặn bởi khoảng cách)
        ...
        -- Kiểm tra đồng xu đã mất chưa (đã nhặt qua fire không cần teleport)
        if not nearest.Parent then continue end

        -- Bước 2: Nếu vẫn còn → di chuyển mượt (Lerp nhiều bước)
        if nearestDist > 3 then
            local startCF = root.CFrame
            for step = 1, COIN_STEPS do
                if not features.AutoPickCoin then break end
                if not nearest.Parent then break end      -- coin đã biến mất giữa chừng
                local t = step / COIN_STEPS
                pcall(function()
                    root.CFrame = startCF:Lerp(CFrame.new(targetPos), t)
                end)
        ...
        if nearest.Parent then
            tryFire(nearest)
            task.wait(0.2) -- chờ server xác nhận pickup
        end
    end
end)
...
local function CreateFloatBtn(text, color, initPos, onPress)
    ...
    -- ── Drag + Tap tách biệt ──────────────────────────────────────────
    -- Kéo > DRAG_THRESHOLD pixel → drag (không kích hoạt action)
    -- Kéo ≤ DRAG_THRESHOLD pixel → tap → kích hoạt action
    ...
    UserInputService.InputChanged:Connect(function(input)
        ...
        local delta = input.Position - dragStart
        if delta.Magnitude > DRAG_THRESHOLD then didDrag = true end
...
-- ================================================================
print("===== MINHCHIEN MM2 v27 – FIXED =====")
print("✅ [1] Float Buttons: kéo = drag, nhẹ = tap (threshold 10px) – không còn kích hoạt khi kéo")
print("✅ [2] Shoot Killer INSTANT: teleport sát sát thủ → bắn → firetouchinterest → quay về")
print("✅ [3] Kill All ĐỒNG THỜI: XẾP HẾT trước mặt 1 lúc → task.spawn song song, không delay")
print("✅ [4] Lock Aim: snap 60fps, mọi role, + Gun+Tap → Freeze sát thủ 0.6s")
print("✅ [5] Auto Pick Coin ANTI-KICK: 1 xu/chu kỳ, Lerp 8 bước, cooldown 1.5s, fire trước teleport")
print("✅ ESP / Silent Aim / Wall Click / NoClip / Auto Escape / Invisible")
