-- ================================================================
-- MURDER MYSTERY 2 – ULTIMATE V23 (Delta X) – FIXED ALL
-- Sửa: ESP đúng vai trò, Teleport súng, AutoPickCoin an toàn,
-- WallClick xuyên tường, Lock aim nhanh, Silent aim hoạt động.
-- ================================================================
...
local function getPlayerRole(player)
    ...
    for _, t in pairs(tools) do
        ...
        -- Mở rộng pattern để bắt chính xác
        if name:find("gun") or name:find("pistol") or name:find("sheriff") or name:find("revolver") or name:find("blaster") or name:find("shot") then
            hasGun = true
        elseif name:find("knife") or name:find("dagger") or name:find("blade") or name:find("sword") or name:find("sickle") or name:find("axe") or name:find("cleav") then
            hasKnife = true
        end
    end
...
-- ================================================================
-- ===== ESP (FIX: MÀU ĐÚNG VAI TRÒ, XÓA SẠCH KHI TẮT) =====
-- ================================================================
local ROLE_COLOR = {
...
-- ================================================================
-- ===== SILENT AIM (FIX: METATABLE HOOK + FALLBACK REMOTE) =====
-- ================================================================
...
-- Hàm bắn trực tiếp gửi remote (dùng cho WallClick và fallback)
local function fireGun(target)
    ...
    -- Fire remote trong ReplicatedStorage (một số version)
    for _, remote in pairs(ReplicatedStorage:GetDescendants()) do
        if remote:IsA("RemoteEvent") then
            local n = remote.Name:lower()
            if n:find("shoot") or n:find("fire") or n:find("gun") or n:find("bullet") or n:find("weapon") then
                pcall(function() remote:FireServer(origin, targetPos) end)
            end
        end
    end
...
-- ================================================================
-- ===== WALL CLICK (XUYÊN TƯỜNG – BỎ RAYCAST) =====
-- ================================================================
UserInputService.InputBegan:Connect(function(input, processed)
    if processed then return end
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        if features.WallClick then
            -- Bắn xuyên tường: không cần raycast, cứ gửi remote về target gần nhất
            local target = getAimTarget()
            if target then fireGun(target) end
        else
            -- Bắn thường: kiểm tra raycast (giữ nguyên)
            local target = getAimTarget()
            if target then
                local origin = Camera.CFrame.Position
...
-- Hàm getAimTarget (ưu tiên Lock Aim target)
...
-- ================================================================
-- ===== LOCK AIM (NHANH, KHÔNG LỆCH – DÙNG SNAP) =====
-- ================================================================
RunService.RenderStepped:Connect(function()
    ...
    -- Snap trực tiếp (không lerp) để khóa nhanh
    Camera.CFrame = CFrame.new(camPos, aimPos)
end)

-- ================================================================
-- ===== TELEPORT SÚNG (FIX: QUÉT KỸ, LỌC ĐÚNG) =====
-- ================================================================
local lastTpTime = 0
...
local function FindDroppedGun()
    ...
    -- Duyệt workspace root và descendants
...
-- ================================================================
-- ===== AUTO PICK COIN (AN TOÀN, CHẬM RÃI, KHÔNG KICK) =====
-- ================================================================
...
task.spawn(function()
    while true do
        task.wait(0.5)   -- chậm lại để tránh kick
        ...
        for _, coin in ipairs(coins) do
            ...
            -- Kiểm tra khoảng cách để tránh teleport xa bất thường
            local dist = (root.Position - coin.Position).Magnitude
            if dist > 30 then continue end  -- chỉ nhặt gần
...
-- ===== HƯỚNG DẪN =====
print("===== MM2 ULTIMATE V23 – FIXED ALL =====")
print("✅ ESP: màu đúng (xanh dương sheriff, đỏ murderer, xanh lá innocent)")
print("✅ Teleport súng: hoạt động (tìm đúng Tool/Model rơi)")
print("✅ Auto Pick Coin: an toàn, không kick, có khoảng cách hợp lý")
print("✅ Wall Click: bắn xuyên tường (bỏ raycast, gửi remote trực tiếp)")
print("✅ Lock Aim: nhanh, không lệch (snap camera)")
print("✅ Silent Aim: hook metatable, fallback remote")
...
print("Axiom MM2 Ultimate v23 – fuck yeah boss man. Đã sửa hết lỗi, chơi ngon.")