...
local function ShootMurdererRemote()
    ...
    if not tool or not isGunName(tool.Name) then
        print("[Shoot] Không có súng để bắn (chưa cầm)")
        return
    end

    -- Tìm sát thủ gần nhất
    local murderer, bestDist = nil, math.huge
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character then
            local r   = plr.Character:FindFirstChild("HumanoidRootPart")
    ...
    if not murderer then print("[Shoot] Không tìm thấy sát thủ"); return end
    ...
    -- 1) Dùng fireGun hiện có (fire remote trong tool + RS)
    fireGun(murderer)

    -- 2) Remote bắn qua nil instances (pattern SimpleSpy)
    if getnilinstances then
        pcall(function()
    ...
    -- 3) Kích hoạt tool trực tiếp
    pcall(function() tool:Activate() end)

    -- 4) Fire tất cả remote trong tool với đầy đủ dạng args
    for _, remote in pairs(tool:GetDescendants()) do
        if remote:IsA("RemoteEvent") then
            pcall(function() remote:FireServer(origin, targetPos) end)
    ...
    print(string.format("[Shoot] ✅ Bắn %s (%.1f studs)", murderer.Name, bestDist))
end
...
-- ================================================================
print("===== MINHCHIEN MM2 v25 – FINAL =====")
print("✅ ESP: chỉ viền màu nhạt (bỏ fill đậm)")
print("✅ Teleport Gun V4: FindFirstChild('GunDrop',true) + status check + teleport về")
print("✅ Shoot Sát Thủ: tự cầm súng + fire remote nil + fireGun + tool:Activate")
print("✅ Lock Aim: snap direction, camera tự follow character (không Scriptable)")
print("✅ Silent Aim / Wall Click / NoClip / Auto Escape / Auto Pick Coin")
print("📱 Nút nổi: bật 'Teleport Súng [BTN]' và 'Shoot Sát Thủ [BTN]' trong menu")
