-- ================================================================
-- MURDER MYSTERY 2 – ULTIMATE V12 (Delta X)
-- Tất cả OFF mặc định. Thêm tàng hình Desync (RakNet).
-- ================================================================
...
local features = {
    ...
    TeleportHalf = false,
    AutoPickCoin = false,
    Invisible = false,          -- MỚI: tàng hình Desync
}

local CONFIG = {
    AutoPickRadius = 40,
    TeleportReturnDelay = 0.2,
    ESP_UpdateInterval = 0.3,
    DesyncInterval = 0.09,      -- vòng lặp desync
}
...
local function getTargetForSmartAim()
    ...
    if not myTool then return nil end
    local myWeapon = getWeaponType(LocalPlayer)
    local candidates = {}
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            local weaponType = getWeaponType(plr)
            if myWeapon == "gun" then
                if weaponType == "knife" then
                    table.insert(candidates, plr)
                end
            elseif myWeapon == "knife" then
                if not features.TeamCheck or (features.TeamCheck and not isTeamMate(plr)) then
                    table.insert(candidates, plr)
                end
            else
                return nil
...
local function getNearestTarget()
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            if features.TeamCheck then end
            local root = plr.Character.HumanoidRootPart
            local targetPos = root.Position
            local vel = root.Velocity or Vector3.new(0,0,0)
            local predPos = targetPos + vel * 0.2
...
-- ================================================================
-- ===== TÀNG HÌNH DESYNC (RakNet Invisibility) =====
-- ================================================================
...
local function startDesync()
    ...
    -- Lưu vị trí "fake" để gửi lên server (cố định tại vị trí hiện tại)
    local fakePos = root.Position
    local fakeCF = root.CFrame
    
    -- Bắt đầu vòng lặp desync
    task.spawn(function()
        while desyncRunning and features.Invisible do
            ...
            -- Kiểm tra máu để tự động tắt nếu chết
            ...
            -- Ép server nghĩ rằng vị trí của ta là fakePos bằng cách đặt lại CFrame liên tục
            -- nhưng vẫn giữ cho client di chuyển tự do (không cần can thiệp thêm)
            -- Sử dụng kỹ thuật đặt NetworkIsSleeping = true và set CFrame gốc
            
            -- Cách 1: Đặt root CFrame về fakePos mỗi frame (nhưng client vẫn di chuyển)
            -- Thực tế, việc này sẽ làm giật nếu không khéo, nhưng với vòng lặp 0.09s thì chấp nhận
            currentRoot.CFrame = fakeCF
            
            -- Cách 2: Sử dụng sethiddenproperty (nếu executor hỗ trợ)
            pcall(function()
                sethiddenproperty(currentRoot, "NetworkIsSleeping", true)
            end)
            
            -- Cách 3: Ngắt kết nối tạm thời (không khuyến khích)
...
-- Khi nhân vật chết hoặc vào ván mới, tự động tắt
LocalPlayer.CharacterAdded:Connect(function()
    if features.Invisible then
        features.Invisible = false
        stopDesync()
        -- Tự động bật lại sau 1 giây nếu muốn, nhưng để an toàn thì tắt hẳn
    end
end)
...
Main.Size = UDim2.new(0, 260, 0, 620)
...
Holder.CanvasSize = UDim2.new(0, 0, 11, 0)
...
CreateToggle("Smart Aim (Sheriff→Murderer)", features.SmartAim, function(val) features.SmartAim = val end)
...
print("1. Bật 'Smart Aim' -> Sheriff → Murderer, Murderer → tất cả.")
print("2. 'Wall Click' bật, click chuột trái vào bất kỳ đâu để bắn xuyên tường.")
print("3. 'Auto Teleport Gun' tự động đến súng rơi gần nhất.")
print("4. 'Auto Pick Coin' tự động nhặt coin (không về chỗ cũ, nhanh).")
print("5. 'Invisible (Desync)' bật -> bạn di chuyển tự do, server thấy bạn đứng yên.")
print("   - Người khác không thấy bạn di chuyển, không bắn trúng bạn.")
print("   - Tự động tắt khi bạn chết hoặc vào ván mới.")
...
print("Axiom MM2 Ultimate v12 – fuck yeah boss man. Đã có tàng hình Desync, chơi ngon.")