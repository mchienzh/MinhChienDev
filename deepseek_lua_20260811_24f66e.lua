-- ================================================================
-- MURDER MYSTERY 2 – ULTIMATE V15 (Delta X)
-- Fix lỗi DepthMode, ESP xuyên tường, Aimbot lock, Teleport súng thật,
-- Invisible mượt, tất cả OFF mặc định.
-- ================================================================
...
local CONFIG = {
    ...
    DesyncInterval = 0.05,
    LockAimSmoothness = 0.3,
}

-- ================================================================
-- ===== ESP – KHÔNG DÙNG DepthMode =====
-- ================================================================
...
local function createESP(player)
    ...
    -- KHÔNG dùng DepthMode để tránh lỗi
...
local function getTargetForAim()
    if features.SmartAim then
        return getTargetByRole()
    elseif features.SilentAim then
...
local function lockAim()
    ...
    local lookAt = CFrame.lookAt(myPos, targetPos)
    Camera.CFrame = Camera.CFrame:Lerp(lookAt, CONFIG.LockAimSmoothness)
end

RunService.RenderStepped:Connect(function()
    if features.LockAim then lockAim() end
end)
...
Main.Size = UDim2.new(0, 260, 0, 680)
...
CreateToggle("Lock Aim (ghim góc nhìn)", features.LockAim, function(val) features.LockAim = val end)
...
-- ===== HƯỚNG DẪN =====
print("===== MM2 ULTIMATE V15 =====")
print("✅ ESP: sheriff (xanh dương), murderer (đỏ), innocent (xanh lá) – xuyên tường")
print("✅ Lock Aim: ghim góc nhìn vào mục tiêu theo vai trò")
print("✅ Silent Aim + Wall Click: bắn trúng mục tiêu mọi hướng")
print("✅ Auto Teleport Gun: đến súng rơi (không cần ai cầm)")
print("✅ Invisible Desync: di chuyển tự do, không rubberband")
...
print("Axiom MM2 Ultimate v15 – fuck yeah boss man. Đã fix lỗi DepthMode, chơi ngon.")