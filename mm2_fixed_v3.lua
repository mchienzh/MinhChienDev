...
local function hookPlayer(player)
    ...
    player.CharacterAdded:Connect(function(char)
        ...
        -- Tool thay đổi → cập nhật màu ngay (đúng vai trò)
    ...
    if player.Character then
        local char = player.Character
        char.ChildAdded:Connect(function(child)
            if child:IsA("Tool") then task.wait(0.05) updateESPColor(player) end
        end)
        char.ChildRemoved:Connect(function(child)
            if child:IsA("Tool") then task.wait(0.05) updateESPColor(player) end
        end)
...
-- Tìm súng rơi gần nhất TOÀN MAP (không giới hạn khoảng cách)
local function FindDroppedGun()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        -- Tool rơi tự do (không ai sở hữu)
        if obj:IsA("Tool") and not IsGunOwned(obj) then
            local name = obj.Name:lower()
            -- Chỉ lấy súng (không lấy knife của murderer)
            if name:find("gun") or name:find("pistol") or name:find("sheriff") or name:find("revolver") then
        ...
        -- Tool trong WeaponDisplays (súng trưng bày trên giá)
        if obj:IsA("Model") and obj.Parent and obj.Parent.Name == "WeaponDisplays" then
            local name = obj.Name:lower()
