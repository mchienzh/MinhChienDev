-- ================================================================
-- MURDER MYSTERY 2 – ULTIMATE V8 (Delta X)
-- Fix: Teleport đến súng rơi tự động (khi bật), 
-- TeleportHalf chỉ áp dụng khi bấm nút TELEPORT TO PLAYER.
-- ================================================================
...
local features = {
    ...
    TeleportHalf = false,   -- chỉ ảnh hưởng khi bấm nút teleport
    AutoPickCoin = false,
}
...
-- ===== ESP (giữ nguyên từ v7, đã tối ưu) =====
local espData = {}
...
Players.PlayerAdded:Connect(function(player)
    player.CharacterAdded:Connect(function() if features.ESP then createESP(player) end end)
    hookToolChanges(player)
    if features.ESP and player.Character then createESP(player) end
end)
...
-- ===== AUTO TELEPORT ĐẾN SÚNG RƠI (FIX HOÀN TOÀN) =====
local function teleportToGun()
    ...
    -- Tìm tất cả Tool không có parent là Character (tức đang rơi)
    local guns = {}
    for _, obj in pairs(workspace:GetChildren()) do
        if obj:IsA("Tool") and obj:FindFirstChild("Handle") then
            local name = obj.Name:lower()
            -- Mở rộng tìm kiếm: gun, pistol, sheriff, revolver, even "weapon"
    ...
    -- Nếu không thấy trong workspace, thử tìm trong các folder (ví dụ "Dropped")
    if #guns == 0 then
        for _, folder in pairs(workspace:GetChildren()) do
            if folder:IsA("Folder") and (folder.Name:lower():find("drop") or folder.Name:lower():find("gun")) then
...
-- Khi có tool mới trong workspace
...
-- Định kỳ mỗi 2 giây (đảm bảo luôn bắt được)
task.spawn(function()
...
-- ===== TELEPORT NỬA NGƯỜI (chỉ dùng khi bấm nút) =====
...
local function pickUpCoins()
    ...
    local nearestCoin, nearestDist = nil, 15
    ...
    if nearestCoin then
        local targetPos = nearestCoin.Position + Vector3.new(0, 1, 0)
...
print("Axiom MM2 Ultimate v8 – fuck yeah boss man. Teleport súng đã fix, TeleportHalf chỉ dùng khi bấm nút. Chơi ngon.")