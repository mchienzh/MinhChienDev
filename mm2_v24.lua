...
-- ================================================================
-- ===== LOCK AIM (SCRIPTABLE CAMERA – KHÔNG LỎ) =====
-- ================================================================
RunService.RenderStepped:Connect(function()
    if not features.LockAim then
        -- Khi tắt, trả camera về Custom
    ...
    if not murderer then
        -- Không thấy murderer → camera thường để nhìn xung quanh
        Camera.CameraType = Enum.CameraType.Custom
        return
    end
    ...
    -- Khoá camera vào murderer: Scriptable + snap CFrame mỗi frame
...
-- ================================================================
-- ===== TELEPORT SÚNG (SỬA: BỎ FILTER TÊN GUN, CHỈ LOẠI DAO) =====
-- ================================================================
local KNIFE_PATTERNS = {"knife","dagger","blade","sword","sickle","axe","cleav","machete","scythe","saber"}
...
local function FindDroppedGun()
    ...
    -- Tìm TẤT CẢ Tool rơi – chỉ loại bỏ dao, không lọc tên gun
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Tool") and not isToolOwned(obj) and not isKnifeName(obj.Name) then
...
task.spawn(function()
    while true do
        ...
        if not gunPart or not gunPart.Parent then continue end

        -- Teleport đến gần súng
        root.CFrame = CFrame.new(gunPart.Position + Vector3.new(0, 3, 0))
        task.wait(0.2)

        -- Thử nhiều cách nhặt súng
        local tool = gunPart.Parent
        -- ProximityPrompt
        local pp = gunPart:FindFirstChildOfClass("ProximityPrompt")
               or tool:FindFirstChildOfClass("ProximityPrompt")
        if pp then pcall(function() fireproximityprompt(pp) end) end
        -- ClickDetector
        local cd = gunPart:FindFirstChildOfClass("ClickDetector")
               or tool:FindFirstChildOfClass("ClickDetector")
        if cd then pcall(function() fireclickdetector(cd) end) end
        -- TouchInterest / Touched
        pcall(function()
            local ti = gunPart:FindFirstChild("TouchInterest")
            if ti then firetouchinterest(root, gunPart, 0) end
        end)
    end
end)
...
-- ================================================================
print("===== MINHCHIEN MM2 v24 – FIXED =====")
print("✅ Wall Click: fixed (getAimTarget đặt đúng thứ tự)")
print("✅ Silent Aim: metatable hook fixed")
print("✅ Lock Aim: Scriptable camera, không lỏ nữa")
print("✅ ESP: real-time auto-update, bỏ nút refresh")
print("✅ Teleport Gun: bắt tất cả Tool rơi (không lọc tên)")
print("✅ NoClip: thêm mới")
print("✅ Auto Escape: sát thủ <3m → tele đến player xa nhất")
